...
// v1.8.38: cattura QUALSIASI output prodotto dagli include (bootstrap e dipendenze)
// o da eventuali warning, così da non contaminare/troncare gli export binari.
// Gli export (xlsx/docx) svuotano questo buffer prima di inviare il file;
// le risposte JSON vengono comunque svuotate a fine script.
...
function export_xlsx(string $filename, string $title, array $headers, array $rows): void
{
    ...
    // v1.8.38: sanitizza i valori — rimuove caratteri di controllo e sequenze
    ...
    // ── 8) Output ──
    // v1.8.37: evita corruzione del binario XLSX — svuota ogni output buffer,
    // disattiva la compressione zlib e termina subito dopo l'invio.
...
function export_docx(string $filename, string $title, array $headers, array $rows): void
{
    ...
    // v1.8.38: sanitizza i valori (vedi export_xlsx)
    $san = static function ($v): string {
        $v = (string)$v;
        if (function_exists('mb_convert_encoding')) { $v = @mb_convert_encoding($v, 'UTF-8', 'UTF-8'); }
        return (string)preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F]/', '', $v);
    ...
    // v1.8.37: evita corruzione del binario DOCX
    while (function_exists('ob_get_level') && ob_get_level() > 0) { @ob_end_clean(); }
    @ini_set('zlib.output_compression', '0');
