...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($action) {
            ...
            // ── TOTP disable (l'utente può disabilitare la PROPRIA 2FA) ──
            case 'totp_disable':
                if (!$state['totp_active']) {
                    $flash_error = 'TOTP non attivo, niente da disabilitare.';
                } else {
                    $pdo->prepare(
                        "UPDATE user_2fa
                    ...
                    if (function_exists('write_log')) {
                        write_log('Auth', 'warning', 'TOTP disabilitato dall\'utente', $u_id);
                    }
                    $flash_success = 'TOTP disabilitato. Puoi riconfigurarlo in qualsiasi momento (se l\'autorizzazione admin è ancora attiva).';
                    $state = TwoFactor::getUserState($pdo, $u_id);
                }
                break;
  ...
  <?php else: ?>
    ...
    <div class="sec-card">
      ...
      <?php elseif ($state['totp_active']): ?>
        ...
        <form method="POST" onsubmit="return confirm('Disabilitare TOTP? Dovrai riconfigurarlo se vorrai riattivarlo.')">
          <?= csrf_field() ?>
          <input type="hidden" name="action" value="totp_disable">
          <button type="submit" class="btn" style="background:#fee2e2;color:#991b1b">
            <i class="fa-solid fa-power-off"></i> Disabilita TOTP
          </button>
          <span style="font-size:11px;color:var(--muted);margin-left:8px">Il secret verrà cancellato. Potrai riconfigurare in qualsiasi momento.</span>
        </form>
      <?php else: ?>
