# PortalManager — Technical Design v1.7.91
...
- `project_dashboard.php` → 5 tab: Anagrafica, Effort Presales, Team, Redditività, Consuntivo
  - **Consuntivo (v1.7.91)**: elenco paginato (50/pagina) e filtrabile (testo libero su rapporto/
  ...
  - **Team (v1.7.91)**: colonne *Ore da rapporti*, *Rapporti* e *Costo su rapporti* affiancate alle ore
...
### 5-ter. Dimensionamento colonne (v1.7.91)
...
### 5-quater. Controllo & Riconciliazione import (v1.7.91)
...
### 5-quinquies. Consuntivo e Team (v1.7.91)
...
### 5-sexies. Timesheet e Gantt (v1.7.91)
...
### 5-septies. Console di sistema (v1.7.91)
...
### 5-octies. Carico risorse e sovrapposizioni (v1.7.91)
...
3. **Sovrapposizioni tra commesse** (`projectOverlaps()`): coppie di commesse che condividono le stesse
   risorse negli stessi mesi (contesa). Dalla v1.7.91 rispetta l'intero set di filtri (periodo, commessa,
...
### 5-novies. Cestino / ripristino record (v1.7.91)
...
fallback `display_name`/email). Nella v1.7.91 sono agganciati al cestino i delete di membri team e fasi
...
### 5-novies. Cestino esteso a tutto il portale (v1.7.91)
...
### 5-decies. Import Commesse DB (v1.7.91)
...
### 5-undecies. Anagrafica Professionisti (v1.7.91)
...
### 5-duodecies. Riconciliazione via Professionisti e promozione a Dipendente (v1.7.91)
...
### 5-terdecies. Professionista esterno come tecnico e nel team (v1.7.91)
...
### 5-quaterdecies. Mappatura dei tecnici su Anagrafica Professionisti (v1.7.91)
...
### 5-quindecies. Fasce legate alla singola commessa (v1.7.91)
...
### 5-sexdecies. Estrazione anagrafica dipendenti (v1.7.91)
...
- **Import in streaming (v1.7.91)**: `XlsxReader::each($path, $cb)` legge il foglio a flusso
...
- **`UploadGuard` (v1.7.91)**: se il body POST eccede `post_max_size`, PHP lo scarta e `$_POST`
...
SQL nella root**: `upgrade_1_7_56_to_1_7_91.sql`, consolidamento idempotente delle release
**1.7.56 → 1.7.91**. `system_update.php` estrae i file (sovrascrittura) ed esegue quell'unico
...
| 1.7.90 | applica 1.7.91 (email aziendale) |
...
(`migration_v1_7_56_to_1_7_91.sql`, per SQL Runner).
