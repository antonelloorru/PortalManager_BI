# Release Checklist — v1.7.99 (Zero-omission)
...
## Hotfix export XLSX (1.7.99)
- [x] Fogli costruiti prima di sharedStrings.xml (indici coerenti)
- [x] Riconoscimento numerico canonico: 0,5 e 0,03 numerici; 007 e 0552 testo
- [x] Foglio vuoto senza <cols> vuoto; styles.xml completo
- [x] Validato con parser OOXML su casi limite e sull'export Finance a 29 colonne

## Finance: vista personalizzabile (1.7.98)
- [x] finance_view_prefs (per utente): colonne visibili, ordine, modalita export
- [x] Registro colonne con tipi sql/calc/used e flag di riservatezza
- [x] 17 colonne economiche gated dal permesso riservato HR (tabella, pannello, export)
- [x] Export "tutti i campi" o "come la vista", con override via parametro mode
- [x] Testato: valori coerenti col prospetto, preferenze rilette, XLSX valido, render pulito

## Sezione Finance (1.7.97)
- [x] finance_overview.php con le 12 colonne richieste (lookup azienda/sede/dipartimento)
- [x] Campo employees.agency (Azienda o Agenzia) + editing nell'inquadramento contrattuale
- [x] Filtri multipli + export XLSX/CSV coerente con i filtri, tracciato nel log
- [x] Link a scheda anagrafica e scheda riservata HR, condizionati ai permessi
- [x] Menu Amministrazione, permessi ruoli 1/2/10; render senza warning

## Tipo rapporto Interinale (1.7.96)
- [x] ENUM contract_type esteso con Interinale (ADD IF NOT EXISTS + MODIFY, idempotente)
- [x] Opzione nel select della scheda dipendente + whitelist POST
- [x] Elenco tipi contratto di manage_employees allineato (filtro e modale)
- [x] Testato: salvataggio/rilettura e conservazione dei valori preesistenti

## Formule configurabili (1.7.95)
- [x] hr_formulas + hr_formula_history (idempotenti, 8 formule seed)
- [x] ValoreFTE = CostoNoAuto x Moltiplicatore FTE; modello allineato al prospetto
- [x] FormulaEval: parser sicuro senza eval, validazione al salvataggio, fallback runtime
- [x] Sezione Formule in Valori di riferimento HR con storico e motivo
- [x] Scheda Compensation mostra le formule correnti; render senza warning

## Riferimenti storicizzati e scheda separata (1.7.94)
- [x] hr_reference_values + hr_reference_history (idempotenti, 5 parametri seed)
- [x] Pagina Amministrazione con modifica + storico e motivo; salva solo le variazioni
- [x] CostModel legge dalla tabella (fallback app_settings/default)
- [x] employee_compensation.php separata; profilo con rimando; riferimenti in sola lettura
- [x] CostoNoAuto = FullCost + TotAAxTA+BP + Incentivazione Extra
- [x] Menu Amministrazione e permessi; render senza warning

## Costo pieno e valore FTE (1.7.93)
- [x] 9 colonne employees (precisione 5/4 decimali) idempotenti + 5 riferimenti in app_settings
- [x] app/CostModel.php: 8 formule con fallback ai riferimenti globali
- [x] Sezione Compensation spostata nell'area riservata HR ed estesa; badge RIF.
- [x] Anti-leak esteso ai nuovi campi; POST handler aggiornato
- [x] Testato sui valori del prospetto fornito: tutti i totali coincidono
...
- [x] Root con **un solo** SQL: `upgrade_1_7_56_to_1_7_99.sql` (40 statement, 0 skip indesiderati)
- [x] Consolidamento 1.7.56 → 1.7.99 idempotente; testato da 1.7.56 e da 1.7.59 pre-fix
...
## Documentazione
- [x] `docs/TECHNICAL_DESIGN_v1.7.99.md` (viste, logiche calcolo, moduli, schema ER)
- [x] `docs/MANUALE_ADMIN_v1.7.99.md`
- [x] `docs/MANUALE_UTENTE_v1.7.99.md`
- [x] `docs/DEPLOYMENT_v1.7.99.md`
