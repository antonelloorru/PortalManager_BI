# Manuale Amministratore — v1.7.98 (Gestione Commesse)
...
## Riconciliazione con Professionisti e importazione in Dipendenti (v1.7.98)
- In **Controllo & Riconciliazione**, il pulsante *Ricalcola/Riapplica* ora aggancia i rapporti anche
  usando l'**Anagrafica Professionisti**: i professionisti collegati a un dipendente vengono usati per
  seminare gli alias e per risolvere i nomi grezzi (username, sigla, nome/cognome anche invertiti).
  Suggerimento: dopo aver collegato o importato professionisti, riesegui la riapplicazione.
- In **Anagrafica Professionisti**, per un professionista esterno non ancora in anagrafica dipendenti è
  ora disponibile il pulsante **In Dipendenti**: crea il dipendente a partire dai dati del professionista
  e lo collega. Se l'operatore non è più attivo, viene proposta la **data di cessazione** (modificabile);
  il dipendente viene creato come *inattivo* con quella `end_date`.

## Merge anagrafiche: criterio "Stesso nome (simile)" (v1.7.98)
Nella pagina *Verifica & Merge Anagrafiche Dipendenti* è disponibile il criterio **Stesso nome (simile)**.
Raggruppa i dipendenti che condividono la **prima parola** di cognome e nome, mostrando però solo i casi
in cui il nome completo differisce: è pensato per unire schede in cui manca un **secondo nome** (es.
"Mario Rossi" e "Mario Giuseppe Rossi") o un **cognome composto** (es. "Anna Rossi" e "Anna Rossi
Bianchi"). Come per gli altri criteri, il merge resta manuale: si confrontano le due schede campo per
campo prima di unirle.

## Controllo & Riconciliazione: verifica anagrafiche (v1.7.98)
La pagina mostra ora un pannello **Verifica anagrafiche (Dipendenti + Professionisti)** che confronta i
tecnici non ancora risolti con l'anagrafica Dipendenti aggiornata e con l'anagrafica Professionisti,
classificandoli in: corrispondono a Dipendenti, Professionisti già collegati, Professionisti esterni (da
promuovere), senza corrispondenza. Il pulsante **Allinea ora** riapplica la risoluzione e aggancia
retroattivamente i rapporti risolvibili. Per i professionisti esterni, un avviso rimanda all'Anagrafica
Professionisti per promuoverli in Dipendenti o collegarli; dopo l'operazione basta tornare qui e premere
Allinea ora.

## Professionisti esterni nel team di commessa (v1.7.98)
Se un rapporto di intervento è svolto da un professionista esterno (presente in Anagrafica
Professionisti ma non tra i dipendenti), la riconciliazione lo associa automaticamente al rapporto e
*Popola team dai rapporti* lo include nel team della commessa. In tabella team ogni risorsa riporta ora
il **Tipo**: badge **Dipendente** (viola) o **Esterno** (azzurro). Per gli esterni il costo orario usato
è quello indicato in Anagrafica Professionisti (costo orario); per i dipendenti il costo derivato dalla RAL.

## Mappare un tecnico non risolto a un professionista (v1.7.98)
Nella sezione *Tecnici non risolti* di Controllo & Riconciliazione, il menu **Mappa a** propone ora anche
i professionisti dell'omonima anagrafica, oltre ai dipendenti. Gli esterni sono contrassegnati con
*[professionista esterno]*: scegliendoli, i rapporti con quel nome vengono attribuiti al professionista
(e, se questo è collegato a un dipendente, direttamente al dipendente). Il suggerimento automatico
riconosce anche i nomi con ordine invertito.

## Fasce legate alla singola commessa (v1.7.98)
- **Mappatura per commessa**: in *Fasce non risolte*, oltre al menu generale, ogni sigla mostra
  l'elenco delle commesse in cui compare, con un menu dedicato. Usalo quando la stessa sigla (es. `E`)
  corrisponde a fasce diverse a seconda della commessa. La mappatura di commessa ha la precedenza su
  quella generale.
- **Tariffe per commessa**: nella dashboard della commessa, card *Tariffe di fascia per questa commessa*.
  Lascia *Professionista* su "tutta la commessa" per una tariffa valida per l'intera commessa (fascia
  `E` con valore specifico); indica un professionista per una fascia dedicata a chi esegue l'attività
  (fascia `X`). Ordine di applicazione: commessa + professionista → commessa → tariffa generale di fascia.
  Dopo aver inserito le tariffe, esegui *Riapplica* nella riconciliazione per ricalcolare i valori.

## Estrazione Anagrafica Dipendenti (v1.7.98)
Menu *Anagrafica → Estrazione anagrafica*. Consente di scaricare in **XLSX** o **CSV** i dati
anagrafico-contrattuali dei dipendenti: Cognome, Nome, Codice fiscale, Matricola, Email aziendale, Contratto, Data
assunzione, Data cessazione, Azienda, Sede, Tipo rapporto, Dipartimento, Qualifica/Ruolo,
Classificazione finanziaria, Stato. Sono disponibili i filtri per azienda e per stato (in forza,
cessati, tutti) e un'anteprima delle prime 25 righe.
**Accesso riservato** ad Amministratore (Super Admin), HR Director e al nuovo ruolo **Responsabile
Finanziario**; per assegnare quest'ultimo, usa la gestione utenti. L'estrazione non contiene dati
retributivi ed è registrata nel log di sistema.

## Verifica & Merge: controllo "Email aziendale mancante" (v1.7.98)
Nella pagina *Verifica & Merge Anagrafiche Dipendenti* è disponibile il criterio **Email aziendale
mancante**: elenca i dipendenti che hanno l'email **personale** valorizzata ma l'email **aziendale**
vuota. Per ciascuno puoi decidere se copiare la personale nell'aziendale: seleziona le righe desiderate
(o usa *seleziona tutti*) e premi **Applica ai selezionati**. Nessuna modifica avviene senza selezione
esplicita, i record che hanno già un'email aziendale non vengono mai toccati e l'operazione è registrata
nel log di sistema.

## Compensation & Benefit: costo pieno e valore FTE (v1.7.98)
La sezione è ora collocata nell'area riservata HR della scheda dipendente. Oltre a RAL, premio, km e
indennità, contiene i parametri per il costo pieno: **Moltiplicatore FC**, **Qt. Trasferte Annue**,
**Qt. Buoni Pasto**, **ValoreTABP**, **Val.KM** (accanto a Km concordati), **Incentivazione Extra**,
**Valore Medio anno Auto**, **OverHead Aziendale** e **Moltiplicatore FTE**.
I campi contrassegnati con **RIF.** stanno usando il valore aziendale di riferimento: lasciandoli vuoti
si applica quello globale (impostabile in app_settings: `hr_mult_fc`, `hr_valore_tabp`, `hr_val_km`,
`hr_overhead_aziendale`, `hr_mult_fte`), compilandoli si sovrascrive per il singolo dipendente.
In fondo alla sezione il riquadro **Valori calcolati** mostra FullCost, TotAAxTA+BP, Rimborso KM,
TotalePreOverHead, **TotCostoTab**, CostoNoAuto, ValoreFTE e **TotaleFTE+CA**, aggiornati al salvataggio.

## Valori di riferimento HR e scheda Compensation (v1.7.98)
- **Amministrazione → Valori di riferimento HR**: qui si impostano i parametri globali (Moltiplicatore
  FC, ValoreTABP, Val.KM, OverHead Aziendale, Moltiplicatore FTE). Puoi indicare un *motivo della
  modifica*; ogni variazione finisce nello **storico** con valore precedente, nuovo valore, autore e data.
  Vengono salvate solo le voci effettivamente cambiate.
- **Scheda Compensation & Benefit**: dalla scheda del dipendente si apre con *Apri scheda Compensation*.
  In alto sono riportati i valori di riferimento in sola lettura, con l'indicazione se sono in uso o
  sovrascritti nella scheda; nel modulo sottostante si inseriscono i dati economici del dipendente
  (lasciando vuoto un parametro si applica il riferimento globale). In fondo, i valori calcolati.
- **CostoNoAuto** è ora calcolato come FullCost + TotAAxTA+BP + Incentivazione Extra.

## Formule di calcolo modificabili (v1.7.98)
In *Amministrazione → Valori di riferimento HR*, sotto ai parametri, la tabella **Formule di calcolo**
consente di modificare le espressioni dei valori derivati. Sono ammessi numeri, gli operatori
`+ - * /`, le parentesi e i nomi elencati sopra la tabella (variabili di input e risultati delle formule
precedenti): qualsiasi altro contenuto viene rifiutato e non eseguito. Indicando un motivo, ogni modifica
finisce nello **storico delle formule**. Se una formula contiene un errore di sintassi non viene salvata;
se risultasse non valida in fase di calcolo, viene applicata la definizione predefinita e la scheda
Compensation lo segnala.
**ValoreFTE** è ora calcolato su **CostoNoAuto** (× Moltiplicatore FTE).

## Tipo di rapporto "Interinale" (v1.7.98)
Nell'inquadramento contrattuale (scheda dipendente e modulo di Anagrafica dipendenti) e' disponibile il
tipo di rapporto **Interinale**, accanto a Indeterminato, Determinato, Apprendistato, Somministrazione,
Consulenza, Stage e Partita IVA. E' selezionabile in inserimento e modifica, utilizzabile come filtro
nell'elenco dipendenti e compare nella colonna *Contratto* dell'estrazione anagrafica.

## Sezione Finance (v1.7.98)
Menu *Amministrazione → Finance*. Elenca tutti i dipendenti con Cognome, Nome, Email aziendale,
**Azienda o Agenzia**, Sede, Dipartimento, Qualifica/Ruolo, Tipo di rapporto, Stato, Data assunzione,
Data fine e Classificazione finanziaria.
- **Filtri**: ricerca libera, azienda, agenzia, sede, dipartimento, tipo di rapporto, classificazione
  finanziaria, stato (in forza / cessati) e periodo di assunzione. Il pulsante *Azzera* ripristina la vista.
- **Export**: XLSX e CSV, sempre coerenti con i filtri applicati (CSV con separatore `;` e UTF-8 BOM).
- **Approfondimento**: ogni riga ha i collegamenti alla scheda anagrafica e, per chi ne ha il permesso,
  alla scheda riservata HR *Compensation & Benefit*.
- **Accesso**: Responsabile Finanziario, HR Director e Amministratore; per casi particolari si possono
  abilitare singoli utenti dalla mappa permessi.
- Il campo **Azienda o Agenzia** si compila nella scheda dipendente (Inquadramento contrattuale, campo
  *Azienda o Agenzia (se esterna)*): se vuoto viene mostrata l'azienda del dipendente.

## Finance: personalizzare la vista (v1.7.98)
Nella sezione Finance il pannello **Personalizza colonne e modalità di esportazione** permette di:
- **scegliere le colonne** con le caselle di spunta (29 disponibili: 12 anagrafiche/contrattuali e 17
  economiche);
- **ordinarle** assegnando un numero crescente da sinistra a destra nel campo accanto alla casella;
- decidere se l'**esportazione** debba contenere *tutti i campi* (impostazione predefinita) oppure
  seguire *la vista personalizzata*.
Le impostazioni sono salvate per utente: ogni persona ha la propria vista. Il pulsante *Ripristina
predefinite* riporta alle 12 colonne anagrafiche. In fondo al pannello sono presenti anche link di
esportazione immediata che ignorano la preferenza salvata (XLSX/CSV, tutti i campi o come la vista).
Le colonne economiche — RAL, FullCost, ValoreTABP, Qt. Trasferte Annue, Qt. Buoni Pasto, TotAAxTA+BP,
Km concordati, Val.KM, Rimborso KM, Incentivazione Extra, Valore Medio anno Auto, TotalePreOverHead,
OverHead Aziendale, TotCostoTab, CostoNoAuto, ValoreFTE, TotaleFTE+CA — sono evidenziate in rosso e
visibili **solo** a chi ha il permesso riservato HR.
