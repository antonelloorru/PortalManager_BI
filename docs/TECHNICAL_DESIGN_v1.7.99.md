# PortalManager — Technical Design v1.7.99
...
- `project_dashboard.php` → 5 tab: Anagrafica, Effort Presales, Team, Redditività, Consuntivo
  - **Consuntivo (v1.7.99)**: elenco paginato (50/pagina) e filtrabile (testo libero su rapporto/
  ...
  - **Team (v1.7.99)**: colonne *Ore da rapporti*, *Rapporti* e *Costo su rapporti* affiancate alle ore
...
### 5-ter. Dimensionamento colonne (v1.7.99)
...
### 5-quater. Controllo & Riconciliazione import (v1.7.99)
...
### 5-quinquies. Consuntivo e Team (v1.7.99)
...
### 5-sexies. Timesheet e Gantt (v1.7.99)
...
### 5-septies. Console di sistema (v1.7.99)
...
### 5-octies. Carico risorse e sovrapposizioni (v1.7.99)
...
3. **Sovrapposizioni tra commesse** (`projectOverlaps()`): coppie di commesse che condividono le stesse
   risorse negli stessi mesi (contesa). Dalla v1.7.99 rispetta l'intero set di filtri (periodo, commessa,
...
### 5-novies. Cestino / ripristino record (v1.7.99)
...
fallback `display_name`/email). Nella v1.7.99 sono agganciati al cestino i delete di membri team e fasi
...
### 5-novies. Cestino esteso a tutto il portale (v1.7.99)
...
### 5-decies. Import Commesse DB (v1.7.99)
...
### 5-undecies. Anagrafica Professionisti (v1.7.99)
Gli operatori del gestionale che non figurano tra i dipendenti popolano un'anagrafica separata,
`cm_professionals` (classe `app/ProfessionalStore.php`). L'importer `import_professionals.php` legge
l'export "operator" (CSV separatore `|`) con parser nativo in streaming e UPSERT su
`source_operator_id`; per riservatezza esclude a monte i campi credenziali (`password`,
`temp_password`, `password_history`, `rfid`), importando solo nome, cognome, email, azienda (sigla →
`exec_company_id`), recapiti, costo orario/pieno, skill e note; le righe `deleted=1` sono saltate salvo
opt-in. La scheda `professionals.php` elenca e filtra i professionisti, ne mostra lo stato
(nuovo/confermato/unito/ignorato) e propone il **merge** verso un dipendente esistente tramite
`suggestEmployee()` (match per email, poi nome+cognome, anche invertiti). Il merge (`linkToEmployee()`)
imposta `employee_id`, porta lo stato a *unito* e crea gli **alias tecnico** (nome, nome invertito,
username, sigla) così i rapporti di intervento si agganciano automaticamente al dipendente collegato.
FK `employee_id` → `employees` con `ON DELETE SET NULL`.

### 5-duodecies. Riconciliazione via Professionisti e promozione a Dipendente (v1.7.99)
**Controllo & Riconciliazione** (`import_control.php`, `cm_reapply()`) ora sfrutta l'Anagrafica
Professionisti — che proviene dallo stesso gestionale dei rapporti. In fase di ri-applicazione: (1) semina
gli **alias tecnico** dai professionisti collegati a un dipendente (username, sigla, 'Nome Cognome',
'Cognome Nome' → `employee_id`) con `INSERT IGNORE` sulla UNIQUE `raw_name`; (2) risolve i tecnici anche
via JOIN diretta su `cm_professionals` (professionista collegato → `technician_id` del dipendente). Così i
rapporti dei tecnici presenti tra i professionisti/dipendenti si agganciano automaticamente.
**Promozione a Dipendente** (`ProfessionalStore::promoteToEmployee()`): crea un record `employees` dai dati
del professionista (rileva via `information_schema` le colonne presenti; mappa nome, email, telefono,
azienda, sigla→employee_code, note), imposta `status` = active/inactive e, se fornita, la **data di
cessazione** `end_date` (proposta automaticamente quando l'operatore non è attivo). Al termine collega il
professionista (stato *unito*) e crea gli alias tecnico. Nessuna variazione di schema.

### 5-terdecies. Professionista esterno come tecnico e nel team (v1.7.99)
Un rapporto di intervento può ora essere attribuito a un **professionista esterno** (non presente tra i
dipendenti): nuova colonna `cm_intervention_reports.technician_professional_id` → `cm_professionals`. In
Controllo & Riconciliazione, `cm_reapply()` associa automaticamente i rapporti non risolti ai
professionisti esterni (match su username/sigla/nome/nome invertito) valorizzando quella colonna, solo
quando non esiste un dipendente corrispondente. La tabella `cm_team` ospita sia dipendenti sia esterni:
`employee_id` reso nullable, nuove colonne `professional_id` (+ UNIQUE per commessa) e `member_type`
('dipendente'/'esterno'). `ProjectModel::syncTeamFromReports()` popola il team da entrambe le fonti;
`ProjectModel::team()` risolve nome e costo orario da employees o da cm_professionals (per gli esterni usa
`hourly_cost` del professionista) ed espone `member_type`, mostrato in dashboard con un badge
Dipendente/Esterno. Migrazione idempotente; nessun impatto sui team già esistenti (marcati 'dipendente').

### 5-quaterdecies. Mappatura dei tecnici su Anagrafica Professionisti (v1.7.99)
Nella riconciliazione, il menu **Mappa a** dei tecnici non risolti elenca ora sia i **dipendenti** sia i
**professionisti**. I professionisti collegati a un dipendente compaiono con l'id del dipendente; gli
**esterni** usano la chiave `P<id>` ed etichetta con suffisso *[professionista esterno]*. La scelta è
persistita in `cm_alias_technician.professional_id` (nuova colonna): in `cm_reapply()` un alias con
`professional_id` risolve su `technician_id` se il professionista è collegato a un dipendente, altrimenti
su `technician_professional_id`. Lo stesso vale per il reimport del foglio mappature, che accetta id nella
forma `P<id>`; il foglio di riferimento esportato riporta ora le colonne *id / risorsa / tipo*.
`cm_suggest()` confronta anche la forma con **token ordinati**, così i nomi invertiti ("Mario Rossi" vs
"Rossi Mario", prima 45%) vengono correttamente suggeriti.

### 5-quindecies. Fasce legate alla singola commessa (v1.7.99)
Due meccanismi distinti, entrambi con scope di commessa:
1. **Alias di fascia per commessa** — `cm_alias_band.project_id` (0 = globale), UNIQUE su
   `(raw_band, project_id)`. La stessa sigla (es. `E`) può essere mappata su fasce diverse a seconda
   della commessa. In `cm_reapply()` la priorità è: alias di commessa → nome fascia → alias globale.
   `AliasStore::all()` per le fasce considera solo gli alias globali (`project_id = 0`), così la
   risoluzione generale non viene inquinata da quelle di commessa.
2. **Tariffe di fascia per commessa/professionista** — `cm_project_band_rates`
   (project_id, band_id, professional_id, cost_type, regime, rate_hour), con `professional_id = 0` per
   l'intera commessa. Copre sia la fascia `E` con valore diverso per commessa sia la fascia `X`
   aggiuntiva legata al professionista che esegue l'attività. `RateResolver::rateFor()` e
   `calcCostsFor()` applicano la priorità **commessa+professionista → commessa → tariffa generale**,
   con il consueto fallback Reperibilità → Ordinario; lo stesso ordine è replicato nel ricalcolo
   set-based di `cm_reapply()` (LEFT JOIN + COALESCE), che usa `technician_professional_id` del rapporto.
La UI espone la mappatura per commessa sotto ogni fascia non risolta e una card *Tariffe di fascia per
questa commessa* nella dashboard di commessa.

### 5-sexdecies. Estrazione anagrafica dipendenti (v1.7.99)
`export_employees.php` produce l'estrazione anagrafico-contrattuale in **XLSX** (via `XlsxWriter`) e
**CSV** (separatore `;`, UTF-8 con BOM per Excel). Le colonne sono fisse: Cognome, Nome, Codice fiscale,
Matricola, Email aziendale, Contratto, Data assunzione, Data cessazione, Azienda, Sede, Tipo rapporto, Dipartimento,
Qualifica/Ruolo, Classificazione finanziaria, Stato. Le denominazioni provengono dalle lookup
(`companies`, `company_locations`, `work_modes`, `departments`); *Qualifica/Ruolo* usa `qualification`
con fallback su `job_title`, *Dipartimento* la tabella `departments` con fallback sul campo testuale.
L'*Email aziendale* proviene da `business_email` (con fallback su `work_email`/`email`).
Le colonne effettivamente presenti in `employees` sono rilevate via `information_schema`, così la pagina
si adatta a schemi parziali. Filtri: azienda e stato (in forza / cessati / tutti). **Nessun dato
retributivo** viene esportato. Accesso riservato tramite permessi di pagina ai ruoli Super Admin (1),
HR Director (2) e al nuovo ruolo **Responsabile Finanziario** (id 10), creato dalla migrazione solo se
assente; il download richiede anche il permesso `export`. Ogni estrazione è tracciata in `app_logs`.

### 5-septendecies. Costo pieno e valore FTE (v1.7.99)
La sezione **Compensation & Benefit** è stata spostata nell'area riservata HR della scheda dipendente
(subito prima di *Note riservate HR*) ed estesa con i campi per il calcolo del costo pieno.
**Campi manuali** (colonne di `employees`): `moltiplicatore_fc` (5 decimali), `qt_trasferte_annue`,
`qt_buoni_pasto`, `valore_tabp`, `val_km` (4 decimali), `incentivazione_extra`,
`valore_medio_anno_auto`, `overhead_aziendale` (quota, es. 0,03), `moltiplicatore_fte` (4 decimali).
**Valori di riferimento validi per tutti** in `app_settings` (`hr_mult_fc` 1,42540, `hr_valore_tabp`
46,48, `hr_val_km` 0,5000, `hr_overhead_aziendale` 0,0300, `hr_mult_fte` 0,1238): usati come default
quando il campo del dipendente è vuoto, con badge **RIF.** in interfaccia.
**Campi calcolati** (non memorizzati, derivati a runtime da `app/CostModel.php`):
FullCost = RAL × Moltiplicatore FC; TotAAxTA+BP = (Qt. Buoni Pasto + Qt. Trasferte Annue) × ValoreTABP;
Rimborso KM = Km concordati × Val.KM; TotalePreOverHead = FullCost + TotAAxTA+BP + Rimborso KM +
Incentivazione Extra + Valore Medio anno Auto; TotCostoTab = TotalePreOverHead × (1 + OverHead Aziendale);
CostoNoAuto = FullCost + TotAAxTA+BP + Incentivazione Extra; ValoreFTE = CostoNoAuto × Moltiplicatore FTE;
TotaleFTE+CA = TotCostoTab + ValoreFTE. I nuovi campi rientrano nell'anti-leak server-side
(`unset`) per gli utenti privi del permesso compensation.

### 5-duodevicies. Valori di riferimento storicizzati e scheda Compensation separata (v1.7.99)
I parametri globali del calcolo costo/FTE non risiedono più in `app_settings` ma in
**`hr_reference_values`** (chiave, etichetta, valore DECIMAL(18,6), decimali, descrizione), gestiti dalla
pagina `hr_reference_values.php` in *Amministrazione*. Ogni variazione è registrata in
**`hr_reference_history`** (valore precedente, nuovo, motivo, autore, data): la pagina mostra lo storico
completo e salva solo le voci effettivamente variate. `CostModel::refs()` legge dalla tabella con
fallback ad `app_settings` (schema 1.7.93) e infine ai default di fabbrica, così l'aggiornamento è
retro-compatibile.
La sezione **Compensation & Benefit** è ora una **pagina dedicata** (`employee_compensation.php?id=N`),
riservata HR: la scheda dipendente espone solo un rimando. Nella pagina i valori di riferimento sono
mostrati in sola lettura (con indicazione *in uso* / *sovrascritto in scheda*) e sono modificabili solo
dalla pagina di Amministrazione. Formula aggiornata: **CostoNoAuto = FullCost + TotAAxTA+BP +
Incentivazione Extra** (coincide con la colonna «Costo senza auto» del prospetto di riferimento).

### 5-undevicies. Formule di calcolo configurabili (v1.7.99)
Le otto formule del modello costo/FTE non sono più cablate nel codice: risiedono in **`hr_formulas`**
(chiave, etichetta, espressione, ordine) e sono modificabili dalla sezione *Valori di riferimento HR*;
ogni variazione è storicizzata in **`hr_formula_history`** (espressione precedente, nuova, motivo, autore,
data). `CostModel::compute()` valuta le espressioni **nell'ordine definito**, rendendo disponibili come
variabili sia gli input (`ral`, `mult_fc`, `trasferte`, `buoni`, `valore_tabp`, `km`, `val_km`,
`incentivo`, `auto`, `overhead`, `mult_fte`) sia i risultati già calcolati.
**Sicurezza**: la valutazione usa `app/FormulaEval.php`, un parser a discesa ricorsiva che accetta
esclusivamente numeri, nomi noti, gli operatori `+ - * /` e le parentesi — nessun `eval()`, nessuna
chiamata di funzione. Le espressioni sono validate al salvataggio (quelle errate non vengono scritte) e,
in caso di errore a runtime, il calcolo ricade sulla definizione predefinita segnalandolo in interfaccia.
`ValoreFTE` è ora definita come **CostoNoAuto × Moltiplicatore FTE**, allineando l'intero modello al
prospetto di riferimento.

### 5-vicies. Sezione Finance (v1.7.99)
`finance_overview.php` (menu *Amministrazione → Finance*) espone il quadro completo del personale per il
controllo di gestione: Cognome, Nome, Email aziendale, **Azienda o Agenzia**, Sede, Dipartimento,
Qualifica/Ruolo, Tipo di rapporto, Stato, Data assunzione, Data fine, Classificazione finanziaria.
Le denominazioni derivano dalle lookup (`companies`, `company_locations`, `departments`); *Azienda o
Agenzia* usa il nuovo campo `employees.agency` (agenzia di provenienza per interinali, somministrati ed
enti esterni) con ricaduta sull'azienda quando non valorizzato; *Qualifica/Ruolo* usa `qualification` con
fallback su `job_title`. Le colonne presenti sono rilevate via `information_schema`.
Filtri: ricerca testuale, azienda, agenzia, sede, dipartimento, tipo di rapporto, classificazione
finanziaria, stato (in forza/cessati) e intervallo di assunzione; l'**export XLSX/CSV** rispetta i filtri
attivi ed è tracciato in `app_logs`. Da ogni riga si raggiungono la **scheda anagrafica** e, per chi ha il
permesso, la **scheda riservata HR Compensation & Benefit**; la vista non contiene dati retributivi.
Permessi assegnati a Super Admin, HR Director e Responsabile Finanziario (a quest'ultimo anche
`employee_profile.php`, `employee_compensation.php` e il permesso virtuale compensation); restano
gestibili per singolo utente dalla mappa permessi.

### 5-unetvicies. Finance: vista personalizzabile e colonne economiche (v1.7.99)
Le colonne della sezione Finance sono descritte in un **registro** (`fin_registry()`) che, per ciascuna,
indica etichetta, tipo e riservatezza: `sql` (espressione SQL), `calc` (valore derivato da `CostModel`,
es. FullCost, TotCostoTab, ValoreFTE) e `used` (parametro effettivamente applicato — riferimento globale
oppure override del dipendente — es. ValoreTABP, Val.KM, OverHead Aziendale). Le 17 colonne economiche
sono marcate come riservate e vengono **escluse dal registro disponibile** per chi non ha il permesso
riservato HR: non compaiono in tabella, nel pannello di personalizzazione né nell'export.
Le preferenze (ordine completo, colonne visibili, modalità di export) sono persistite per utente in
**`finance_view_prefs`** come JSON. Il pannello *Personalizza colonne* usa flag di visibilità e un campo
numerico d'ordine (nessuna dipendenza JavaScript, coerentemente con le scelte del portale).
L'**esportazione** ha due modalità: *tutti i campi* (default) oppure *come la vista personalizzata*;
la preferenza è salvata, ma è possibile forzarla al volo con il parametro `mode`. In entrambi i casi i
filtri attivi sono rispettati. Per efficienza i valori derivati sono calcolati una sola volta per riga e
solo quando almeno una colonna `calc`/`used` è richiesta; i campi sorgente selezionati sono filtrati in
base alle colonne realmente presenti in `employees`.
...
- **Import in streaming (v1.7.99)**: `XlsxReader::each($path, $cb)` legge il foglio a flusso
...
- **`UploadGuard` (v1.7.99)**: se il body POST eccede `post_max_size`, PHP lo scarta e `$_POST`
...
SQL nella root**: `upgrade_1_7_56_to_1_7_99.sql`, consolidamento idempotente delle release
**1.7.56 → 1.7.99**. `system_update.php` estrae i file (sovrascrittura) ed esegue quell'unico
...
| 1.7.79 | applica 1.7.80 e 1.7.81 |
| 1.7.80 | applica 1.7.81 e 1.7.82 |
| 1.7.81 | applica 1.7.82 e 1.7.83 |
| 1.7.82 | applica 1.7.83 e 1.7.84 |
| 1.7.83 | applica 1.7.84 e 1.7.85 |
| 1.7.84 | applica 1.7.85 e 1.7.86 |
| 1.7.85 | applica 1.7.86 e 1.7.87 |
| 1.7.86 | applica 1.7.87 e 1.7.88 |
| 1.7.87 | applica 1.7.88 e 1.7.89 |
| 1.7.88 | applica 1.7.89 e 1.7.90 |
| 1.7.89 | applica 1.7.90 e 1.7.91 |
| 1.7.90 | applica 1.7.91 e 1.7.92 |
| 1.7.91 | applica 1.7.92 e 1.7.93 |
| 1.7.92 | applica 1.7.93 e 1.7.94 |
| 1.7.93 | applica 1.7.94 e 1.7.95 |
| 1.7.94 | applica 1.7.95 e 1.7.96 |
| 1.7.95 | applica 1.7.96 e 1.7.97 |
| 1.7.96 | applica 1.7.97 e 1.7.98 |
| 1.7.97 | applica 1.7.98 e 1.7.99 |
| 1.7.98 | applica 1.7.99 (hotfix export XLSX) |
...
(`migration_v1_7_56_to_1_7_99.sql`, per SQL Runner).
