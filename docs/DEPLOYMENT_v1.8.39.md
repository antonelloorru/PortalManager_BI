# PortalManager — Guida al Deployment v1.8.39
## Aggiornamento a v1.8.39
Da **v1.8.11** già applicata: eseguire `sql/migration_v1_8_39.sql` (re-seed permessi DGB + bump; nessuna variazione di schema). NB (precedente): eseguire `sql/migration_v1_8_11.sql` (aggiunge `cm_projects.dgb_contract_id`, popola la mappatura da external_link e crea la vista `vw_dgb_commessa_rollup`). Aggiornare anche `app/DgbModel.php`, `project_dashboard.php`, `manage_projects.php`, `dgb_activities.php`.
Da **v1.8.0/1.8.1**: eseguire in ordine le migrazioni successive fino a `migration_v1_8_3.sql`.
Da **v1.7.99** o precedenti: eseguire in ordine `migration_v1_8_0.sql`, `migration_v1_8_1.sql`, `migration_v1_8_2.sql`, `migration_v1_8_3.sql`, `migration_v1_8_4.sql`, `migration_v1_8_5.sql`, `migration_v1_8_6.sql`, `migration_v1_8_39.sql`, oppure il consolidato `sql/upgrade_1_7_56_to_1_8_39.sql`.
...
### 1. Contenuto del pacchetto `PortalManager_v1.8.39.zip`
```
app/CostModel.php                 (modificato — calcolo anno-aware)
app/MenuManager.php               (modificato — voci menu)
app/Version.php                   (modificato — PM_VERSION 1.8.39)
...
sql/upgrade_1_7_56_to_1_8_39.sql       (consolidato root per system_update)
VERSION                           (1.8.12)
CHANGELOG.md
...
Expand-Archive -Path .\PortalManager_v1.8.39.zip -DestinationPath P:\xampp\htdocs\portalmanager -Force
```
...
3. In alternativa, **Aggiorna sistema** (`system_update.php`) applica il consolidato
   `sql/upgrade_1_7_56_to_1_8_39.sql`.
...
- Footer/Impostazioni: versione **1.8.39**.
...
- I file `.sql` (migrazioni e consolidato `upgrade_1_7_56_to_1_8_39.sql`) sono in `sql/`.
...
1. File modificati: `sql/migration_v1_8_39.sql`, `app/DgbSync.php` (nuovo), `app/DgbModel.php`, `dgb_activities.php`.
...
## Import dati economici (v1.8.20)
File modificati: `import_economics_xlsx.php` (import in due fasi con anteprima/conferma) e `finance_overview.php` (colonna Codice fiscale). Sovrascriverli entrambi. Il flusso ora e': Verifica e anteprima -> Conferma importazione.


## Import Cognome/Nome (v1.8.21)
File modificato: `import_economics_xlsx.php` (template con Cognome/Nome + match per nominativo). Sovrascriverlo. Riscaricare il template per avere le nuove colonne.


## Pulizia file SQL (v1.8.22)
File modificato: `system_console.php`. Sovrascriverlo. Il tool e' in Console di sistema -> tab SQL -> Pulizia file SQL obsoleti (Super Admin).


## DGB contratti (v1.8.23)
File modificati: `app/DgbImporter.php`, `app/DgbSync.php`, `dgb_activities.php` + migration dgb_forms_contract. Dopo l'aggiornamento: importa il CSV forms_contract in Attività & Rendicontazione DGB -> Import, poi premi Sincronizza per allineare codice/nome delle commesse.


## Hotfix contratti DGB (v1.8.24)
File modificati: `app/DgbImporter.php`, `dgb_activities.php`. Reimportare il CSV forms_contract e premere Sincronizza: gli header maiuscoli (Code/Code_X_Installation) ora vengono riconosciuti.


## Sync DGB non distruttiva (v1.8.25)
File modificato: `app/DgbSync.php`. Dopo Sincronizza, i codici e nomi reali delle commesse restano invariati; i contratti vengono collegati per codice.


## Hotfix lettura XLSX (v1.8.26)
File modificato: `import_economics_xlsx.php`. Sovrascriverlo per risolvere l'errore di lettura dei file XLSX.


## Dipartimenti sottocategorie (v1.8.27)
File modificati: `manage_departments.php`, `manage_employees.php`, `employee_profile.php` + migration parent_id. In Dipartimenti crea la sottocategoria scegliendo il padre; assegnala al dipendente dalla select Dipartimento.


## Sotto-categorie 1-a-molti (v1.8.28)
File modificati: `manage_departments.php`, `manage_employees.php`, `employee_profile.php` + migration. In Dipartimenti crea le sotto-categorie legandole a una categoria; nel dipendente assegna Dipartimento e Sotto-categoria.


## Tipologia sotto-categorie (v1.8.29)
File modificati: `manage_departments.php`, `manage_employees.php`, `employee_profile.php` + migration. Nella gestione sotto-categorie imposta la Tipologia (default: come categoria).


## Organigramma (v1.8.30)
File nuovi/modificati: `organigramma.php` (nuovo), `manage_departments.php`, `app/Router.php`, `app/MenuManager.php` + migration. In Dipartimenti imposta l'Unita superiore delle unita; apri Organigramma dal menu o dal pulsante, filtra e usa Esporta SVG.


## Hotfix tipologia (v1.8.31)
File modificato: `manage_departments.php`. Sovrascriverlo: la tipologia torna modificabile nella riga di modifica dei dipartimenti.


## Hotfix gerarchia unita (v1.8.32)
File modificato: `manage_departments.php`. Sovrascriverlo: l'unita superiore viene ora salvata anche cambiando solo il padre.


## Anagrafe: HR+contrattuale, filtri/export (v1.8.33)
File modificato: `manage_employees.php`. Inquadramento contrattuale ora dentro Inquadramento HR; nel pannello Filtri avanzati e nell'export sono disponibili tutti i campi dato.


## Contrattuale+Compensation in HR (v1.8.34)
File modificato: `manage_employees.php`. Le sezioni sono ora dentro il contenitore Inquadramento HR.


## Scheda dipendente (v1.8.35)
File modificato: `employee_profile.php`. Aprendo un dipendente dall'icona matita, tab Anagrafica: contrattuale/badge/compensation ora dentro il contenitore Inquadramento HR.


## Maschere HR nel tab Inquadramento HR (v1.8.36)
File modificato: `employee_profile.php`. Aprendo un dipendente (matita), le maschere Inquadramento contrattuale/Badge/Compensation sono ora nel tab Inquadramento HR.


## Hotfix export XLSX/DOCX (v1.8.37)
File modificato: `app/saved_views_api.php`. Dopo l'aggiornamento, l'export XLSX/DOCX delle liste (Anagrafica dipendenti e tutte le liste con ListFilter) torna apribile.


## Hotfix export XLSX/DOCX (v1.8.38)
File modificato: `app/saved_views_api.php`. Dopo l'aggiornamento e Ctrl+F5, l'export XLSX dell'Anagrafe dipendenti si apre correttamente.


## Hotfix export (v1.8.39)
File modificato: `saved_views_api.php` (ROOT, non app/). Sovrascriverlo a root. Dopo Ctrl+F5 l'export XLSX dell'Anagrafe dipendenti si apre.
