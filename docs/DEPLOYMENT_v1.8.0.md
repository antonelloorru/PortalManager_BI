# PortalManager — Guida al Deployment v1.8.0
## Aggiornamento da v1.7.99 (o precedenti della serie 1.7.x)
...
### 1. Contenuto del pacchetto `PortalManager_v1.8.0.zip`
```
app/CostModel.php                 (modificato — calcolo anno-aware)
app/MenuManager.php               (modificato — voci menu)
app/Version.php                   (modificato — PM_VERSION 1.8.0)
...
sql/migration_v1_8_0.sql          (nuovo — migrazione idempotente)
upgrade_1_7_56_to_1_8_0.sql       (consolidato root per system_update)
VERSION                           (1.8.0)
CHANGELOG.md
...
Expand-Archive -Path .\PortalManager_v1.8.0.zip -DestinationPath P:\xampp\htdocs\portalmanager -Force
```
...
2. Eseguire lo script `sql/migration_v1_8_0.sql` (idempotente, ri-eseguibile).
3. In alternativa, **Aggiorna sistema** (`system_update.php`) applica il consolidato
   `upgrade_1_7_56_to_1_8_0.sql`.
...
- Footer/Impostazioni: versione **1.8.0**.
...
- **Confronto annualità** e **Import dati economici** accessibili ai ruoli abilitati.
