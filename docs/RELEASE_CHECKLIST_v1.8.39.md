# Release Checklist — v1.8.39 (Zero-omission)
## Carico risorse e sovrapposizioni

## Hotfix v1.8.39 — export: corretto il file REALE (saved_views_api.php a ROOT)
- [x] File reale scaricato analizzato: 190 byte di Notice PHP (session_start Ignoring...) in testa; ZIP dopo il preambolo valido (7 parti)
- [x] Endpoint reale identificato: saved_views_api.php a ROOT (v1.7.36), non app/saved_views_api.php
- [x] `saved_views_api.php` ROOT copiato dal backup + fix: guardia session_start (session_status ACTIVE), ob_start iniziale, export_xlsx/docx buffer+zlib+exit+sanitize
- [x] Rimosso dalla release il file errato app/saved_views_api.php
- [x] Test scenario reale (sessione attiva, display_errors on): 0 Notice a schermo; file NON contiene 'Notice'/'session_start'; PK magic; zip valido 7 parti; openpyxl apre foglio 30 colonne
- [x] `db_upgrade.php` metadata '1.8.39' + UPGRADE_SQL; Version.php/VERSION=1.8.39; CHANGELOG
- [x] Nessuna variazione di schema; 0 commenti con ';'
- [x] QA tokenizer migration RUN1/RUN2 ok; consolidato naive upgrade_1_7_56_to_1_8_39.sql 249 stmt RUN1/RUN2 err=0 (idempotente)


## Hotfix v1.8.38 — export XLSX/DOCX: cause residue (troncamento + XML invalido)
- [x] `app/saved_views_api.php`: ob_start() a inizio file (cattura output di bootstrap e include) -> export svuota e invia binario integro (Content-Length corretto)
- [x] Risposte JSON (list/save/delete/auth) non impattate: buffer svuotato a fine script/exit
- [x] Sanitizzazione dati in export_xlsx e export_docx: rimozione caratteri di controllo e UTF-8 non validi
- [x] Test integrazione: leak simulato (BOM+whitespace+HTML) NON presente nel file; PK magic; zip valido 7 parti; openpyxl apre foglio con 30 colonne; carattere di controllo sanitizzato
- [x] `db_upgrade.php` metadata '1.8.38' + UPGRADE_SQL; Version.php/VERSION=1.8.38; CHANGELOG
- [x] Nessuna variazione di schema; 0 commenti con ';'
- [x] QA tokenizer migration RUN1/RUN2 ok; consolidato naive upgrade_1_7_56_to_1_8_38.sql 248 stmt RUN1/RUN2 err=0 (idempotente)


## Hotfix v1.8.37 — export XLSX/DOCX apribile
- [x] `app/saved_views_api.php` copiato dal backup reale (fonte di verita) + fix
- [x] export_xlsx(): while ob_end_clean + @ini_set zlib.output_compression 0 + exit dopo readfile/unlink
- [x] export_docx(): stesso fix
- [x] Test reale: XLSX generato = OOXML valido (PK magic, zip testzip None, 7 parti), openpyxl apre foglio Anagrafica con header e righe corrette
- [x] Nessun BOM/whitespace prima di <?php; chiusura file pulita
- [x] `db_upgrade.php` metadata '1.8.37' + UPGRADE_SQL; Version.php/VERSION=1.8.37; CHANGELOG
- [x] Nessuna variazione di schema; 0 commenti con ';'
- [x] QA tokenizer migration RUN1/RUN2 ok; consolidato naive upgrade_1_7_56_to_1_8_37.sql 247 stmt RUN1/RUN2 err=0 (idempotente)


## Feature v1.8.36 — maschere HR spostate nel tab Inquadramento HR
- [x] `employee_profile.php`: card Inquadramento contrattuale + Badge + Compensation link spostate dal tab Anagrafica al tab Inquadramento HR
- [x] save_inquadramento esteso: salva agency, ccnl, qualification, contract_level, part_time, part_time_pct, apprenticeship_end_date, gender, badge_number, badge_issue_date
- [x] Rimossi duplicati contract_type/hire_date/end_date dalla card spostata (restano in Ruolo e contratto)
- [x] save_anagrafica: campi spostati preservati con valori $emp (no azzeramento); update compensation guardato da isset($_POST['ral'])
- [x] Test DB: save_inquadramento persiste ccnl/badge/qualifica; save_anagrafica preserva (nessun azzeramento)
- [x] Form anagrafica e inquadramento div bilanciati; nessun name duplicato (contract_type/hire_date/end_date/ccnl/badge = 1)
- [x] `db_upgrade.php` metadata '1.8.36' + UPGRADE_SQL; Version.php/VERSION=1.8.36; CHANGELOG
- [x] Nessuna variazione di schema; 0 commenti con ';'
- [x] QA tokenizer migration RUN1/RUN2 ok; consolidato naive upgrade_1_7_56_to_1_8_36.sql 246 stmt RUN1/RUN2 err=0


## Hotfix v1.8.35 — employee_profile: contrattuale/compensation in contenitore HR
- [x] Identificato che l'editing dipendente usa employee_profile.php (matita), non il modal di manage_employees
- [x] Backup utente confermato == release v1.8.34 (md5 manage_employees identico) -> modal gia ok, mancava employee_profile
- [x] `employee_profile.php` tab Anagrafica: contenitore full-width "Inquadramento HR" attorno a contrattuale+badge+compensation; apertura/chiusura incondizionate
- [x] Nessuna modifica alla logica save_anagrafica (solo HTML wrapping)
- [x] Verifica: form save_anagrafica div bilanciati 51=51; contenitore contiene Inquadramento contrattuale, Compensation & Benefit, Badge; Salva dopo la chiusura
- [x] `db_upgrade.php` metadata '1.8.35' + UPGRADE_SQL; Version.php/VERSION=1.8.35; CHANGELOG
- [x] Nessuna variazione di schema; 0 commenti con ';'
- [x] QA tokenizer migration RUN1/RUN2 ok; consolidato naive upgrade_1_7_56_to_1_8_35.sql 245 stmt RUN1/RUN2 err=0


## Hotfix v1.8.34 — contrattuale+compensation dentro contenitore HR
- [x] `manage_employees.php`: box contenitore attorno a Inquadramento HR + Inquadramento contrattuale + Compensation & Benefit
- [x] Rimossi i border-top separatori di contrattuale e compensation (ora sottosezioni)
- [x] Badge resta sezione distinta fuori dal contenitore
- [x] Verifica div contenitore bilanciato (35=35); sezioni contrattuale e compensation dentro, Badge fuori
- [x] `db_upgrade.php` metadata '1.8.34' + UPGRADE_SQL; Version.php/VERSION=1.8.34; CHANGELOG
- [x] Nessuna variazione di schema; 0 commenti con ';'
- [x] QA tokenizer migration RUN1/RUN2 ok; consolidato naive upgrade_1_7_56_to_1_8_34.sql 244 stmt RUN1/RUN2 err=0


## Feature v1.8.33 — Anagrafe: HR+contrattuale, filtri/export ampliati
- [x] `manage_employees.php`: Inquadramento contrattuale reso sottosezione di Inquadramento HR (rimosso border-top separatore)
- [x] Colonne nascoste (display:none) con tutti i campi: Matricola, CF, Email az., Genere, Azienda, Dipartimento, Sotto-categoria, Sede, Modalita, CCNL, Qualifica CCNL, Livello, Tipo contratto, Part-time, %PT, Scad. apprendistato, Assunzione, Fine, Badge, Ruolo, Email account, RAL (se permesso)
- [x] ListFilter costruisce filtri da tutte le <th> (anche nascoste) ed esporta tutte le celle -> piu filtri + export completo, vista invariata
- [x] Query lista con join departments/department_subcategories (dept_name, subcat_name)
- [x] Bilanciamento colonne header/riga: 30 th = 30 td; RAL condizionale coerente su th e td
- [x] `db_upgrade.php` metadata '1.8.33' + UPGRADE_SQL; Version.php/VERSION=1.8.33; CHANGELOG
- [x] Nessuna variazione di schema; 0 commenti con ';'
- [x] QA tokenizer migration RUN1/RUN2 ok; consolidato naive upgrade_1_7_56_to_1_8_33.sql 243 stmt RUN1/RUN2 err=0


## Hotfix v1.8.32 — salvataggio gerarchia unita
- [x] `manage_departments.php`: calcolo di $pu (unita superiore + anti-ciclo) spostato PRIMA della condizione di modifica (prima era dentro il blocco, referenziato indefinito nella condizione)
- [x] Test: cambio solo padre NULL->2 -> condizione TRUE, parent_id=2 persistito
- [x] Anti-ciclo e validazione padre mantenuti
- [x] `db_upgrade.php` metadata '1.8.32' + UPGRADE_SQL; Version.php/VERSION=1.8.32; CHANGELOG
- [x] Nessuna variazione di schema; 0 commenti con ';'
- [x] QA tokenizer migration RUN1/RUN2 ok; consolidato naive upgrade_1_7_56_to_1_8_32.sql 242 stmt RUN1/RUN2 err=0


## Hotfix v1.8.31 — tipologia dipartimento modificabile
- [x] `manage_departments.php`: ripristinato <select name="value_type" required> nella riga di modifica (perso in v1.8.30 con l'aggiunta di Unita superiore)
- [x] Verifica statica: 2 <select value_type> nel file, 0 <td> seguiti direttamente da <option> (nessun orfano); celle allineate all'header a 6 colonne
- [x] Salvataggio value_type gia corretto lato server (UPDATE departments SET ...,value_type=?,...)
- [x] `db_upgrade.php` metadata '1.8.31' + UPGRADE_SQL; Version.php/VERSION=1.8.31; CHANGELOG
- [x] Nessuna variazione di schema; 0 commenti con ';'
- [x] QA tokenizer migration RUN1/RUN2 ok; consolidato naive upgrade_1_7_56_to_1_8_31.sql 241 stmt RUN1/RUN2 err=0
