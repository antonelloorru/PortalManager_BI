# PortalManager — Technical Design v1.7.71
...
- `project_dashboard.php` → 5 tab: Anagrafica, Effort Presales, Team, Redditività, Consuntivo
  - **Consuntivo (v1.7.71)**: elenco paginato (50/pagina) e filtrabile (testo libero su rapporto/
  ...
  - **Team (v1.7.71)**: colonne *Ore da rapporti*, *Rapporti* e *Costo su rapporti* affiancate alle ore
...
### 5-ter. Dimensionamento colonne (v1.7.71)
...
### 5-quater. Controllo & Riconciliazione import (v1.7.71)
...
### 5-quinquies. Consuntivo e Team (v1.7.71)
...
### 5-sexies. Timesheet e Gantt (v1.7.71)
...
### 5-septies. Console di sistema (v1.7.71)
...
### 5-octies. Carico risorse e sovrapposizioni (v1.7.71)
...
- **Import in streaming (v1.7.71)**: `XlsxReader::each($path, $cb)` legge il foglio a flusso
...
- **`UploadGuard` (v1.7.71)**: se il body POST eccede `post_max_size`, PHP lo scarta e `$_POST`
...
SQL nella root**: `upgrade_1_7_56_to_1_7_71.sql`, consolidamento idempotente delle release
**1.7.56 → 1.7.71**. `system_update.php` estrae i file (sovrascrittura) ed esegue quell'unico
...
| 1.7.70 | applica 1.7.71 (carico risorse) |
...
(`migration_v1_7_56_to_1_7_71.sql`, per SQL Runner).
